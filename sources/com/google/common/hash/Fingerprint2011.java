...
final class Fingerprint2011 extends AbstractNonStreamingHashFunction {
    ...
    /* renamed from: K0 */
    private static final long f233K0 = -6505348102511208375L;

    /* renamed from: K1 */
    private static final long f234K1 = -8261664234251669945L;

    /* renamed from: K2 */
    private static final long f235K2 = -4288712594273399085L;

    /* renamed from: K3 */
    private static final long f236K3 = -4132994306676758123L;

    static long hash128to64(long j, long j2) {
        long j3 = (j2 ^ j) * f236K3;
        long j4 = (j ^ (j3 ^ (j3 >>> 47))) * f236K3;
        return (j4 ^ (j4 >>> 47)) * f236K3;
    }
    ...
    private static long fullFingerprint(byte[] bArr, int i, int i2) {
        long load64 = LittleEndianByteArray.load64(bArr, i);
        int i3 = i + i2;
        long load642 = LittleEndianByteArray.load64(bArr, i3 - 16) ^ f234K1;
        long load643 = LittleEndianByteArray.load64(bArr, i3 - 56) ^ f233K0;
        long[] jArr = new long[2];
        long[] jArr2 = new long[2];
        long j = i2;
        weakHashLength32WithSeeds(bArr, i3 - 64, j, load642, jArr);
        weakHashLength32WithSeeds(bArr, i3 - 32, j * f234K1, f233K0, jArr2);
        long shiftMix = load643 + (shiftMix(jArr[1]) * f234K1);
        long rotateRight = Long.rotateRight(shiftMix + load64, 39) * f234K1;
        long rotateRight2 = Long.rotateRight(load642, 33) * f234K1;
        int i4 = i;
        int i5 = (i2 - 1) & (-64);
        while (true) {
            long rotateRight3 = Long.rotateRight(rotateRight + rotateRight2 + jArr[0] + LittleEndianByteArray.load64(bArr, i4 + 16), 37) * f234K1;
            long rotateRight4 = Long.rotateRight(rotateRight2 + jArr[1] + LittleEndianByteArray.load64(bArr, i4 + 48), 42) * f234K1;
            long j2 = rotateRight3 ^ jArr2[1];
            long j3 = rotateRight4 ^ jArr[0];
            long rotateRight5 = Long.rotateRight(shiftMix ^ jArr2[0], 33);
            weakHashLength32WithSeeds(bArr, i4, jArr[1] * f234K1, j2 + jArr2[0], jArr);
            weakHashLength32WithSeeds(bArr, i4 + 32, jArr2[1] + rotateRight5, j3, jArr2);
            i4 += 64;
            i5 -= 64;
            if (i5 == 0) {
                return hash128to64(hash128to64(jArr[0], jArr2[0]) + (shiftMix(j3) * f234K1) + j2, hash128to64(jArr[1], jArr2[1]) + rotateRight5);
            }
    ...
    private static long hashLength33To64(byte[] bArr, int i, int i2) {
        ...
        int i3 = i + i2;
        int i4 = i3 - 16;
        long load642 = LittleEndianByteArray.load64(bArr, i) + ((i2 + LittleEndianByteArray.load64(bArr, i4)) * f233K0);
        ...
        return shiftMix((shiftMix(((j + rotateRight4 + Long.rotateRight(load648, 31) + rotateRight5 + Long.rotateRight(load647, 7)) * f235K2) + ((load646 + load648 + rotateRight3) * f233K0)) * f233K0) + rotateRight3) * f235K2;
    }
    ...
    static long murmurHash64WithSeed(byte[] bArr, int i, int i2, long j) {
        ...
        for (int i5 = 0; i5 < i3; i5 += 8) {
            j2 = (j2 ^ (shiftMix(LittleEndianByteArray.load64(bArr, i + i5) * f236K3) * f236K3)) * f236K3;
        }
        if (i4 != 0) {
            j2 = (LittleEndianByteArray.load64Safely(bArr, i + i3, i4) ^ j2) * f236K3;
        }
        return shiftMix(shiftMix(j2) * f236K3);
    }
}
