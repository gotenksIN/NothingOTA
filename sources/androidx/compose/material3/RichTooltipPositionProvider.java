...
final class RichTooltipPositionProvider implements PopupPositionProvider {
    ...
    public long mo806calculatePositionllwVHH4(IntRect anchorBounds, long j, LayoutDirection layoutDirection, long j2) {
        ...
        if (IntSize.m5210getWidthimpl(j2) + right > IntSize.m5210getWidthimpl(j) && (right = anchorBounds.getLeft() - IntSize.m5210getWidthimpl(j2)) < 0) {
            right = anchorBounds.getLeft() + ((anchorBounds.getWidth() - IntSize.m5210getWidthimpl(j2)) / 2);
        }
        int top = (anchorBounds.getTop() - IntSize.m5209getHeightimpl(j2)) - this.tooltipAnchorPadding;
        if (top < 0) {
