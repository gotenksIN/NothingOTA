...
public final class SnapshotKt {
    ...
    public static final <T> T sync(Functions<? extends T> block) {
        T invoke;
    ...
    public static final <T extends StateRecord> T writableRecord(T t, StateObject state, Snapshot snapshot) {
        ...
        if (snapshot.getReadOnly()) {
            snapshot.mo2512recordModified$runtime_release(state);
        }
        T t2 = (T) readable(t, snapshot.getId(), snapshot.getInvalid$runtime_release());
        if (t2 == null) {
            readError();
    ...
    public static final Map<StateRecord, StateRecord> optimisticMerges(MutableSnapshot mutableSnapshot, MutableSnapshot mutableSnapshot2, SnapshotIdSet snapshotIdSet) {
        ...
        SnapshotIdSet m287or = mutableSnapshot2.getInvalid$runtime_release().set(mutableSnapshot2.getId()).m287or(mutableSnapshot2.getPreviousIds$runtime_release());
        Object[] values = modified$runtime_release.getValues();
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (readable2 != null && (readable = readable(firstStateRecord, id, m287or)) != null && !Intrinsics.areEqual(readable2, readable)) {
