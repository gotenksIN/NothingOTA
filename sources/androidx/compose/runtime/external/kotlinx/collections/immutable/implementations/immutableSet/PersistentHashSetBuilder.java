...
public final class PersistentHashSetBuilder<E> extends AbstractMutableSet<E> implements PersistentSet.Builder<E> {
    ...
    public boolean addAll(Collection<? extends E> elements) {
        ...
        if (persistentHashSet != null) {
            ...
            int size = size();
            TrieNode<E> mutableAddAll = this.node.mutableAddAll(persistentHashSet.getNode$runtime_release(), 0, mutableCounter, this);
            int size2 = (elements.size() + size) - mutableCounter.getCount();
            if (size != size2) {
    ...
    public boolean retainAll(Collection<? extends Object> elements) {
        ...
        if (persistentHashSet != null) {
            MutableCounter mutableCounter = new MutableCounter(0, 1, null);
            int size = size();
            Object mutableRetainAll = this.node.mutableRetainAll(persistentHashSet.getNode$runtime_release(), 0, mutableCounter, this);
            int count = mutableCounter.getCount();
            if (count == 0) {
                clear();
            } else if (count != size) {
    ...
    public boolean removeAll(Collection<? extends Object> elements) {
        ...
        if (persistentHashSet != null) {
            MutableCounter mutableCounter = new MutableCounter(0, 1, null);
            int size = size();
            Object mutableRemoveAll = this.node.mutableRemoveAll(persistentHashSet.getNode$runtime_release(), 0, mutableCounter, this);
            int count = size - mutableCounter.getCount();
            if (count == 0) {
                clear();
            } else if (count != size) {
