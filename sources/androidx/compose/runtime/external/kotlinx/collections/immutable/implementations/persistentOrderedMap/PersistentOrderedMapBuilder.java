...
public final class PersistentOrderedMapBuilder<K, V> extends AbstractMutableMap<K, V> implements PersistentMap.Builder<K, V> {
    ...
    public PersistentMap<K, V> build() {
        ...
        if (build == this.map.getHashMap$runtime_release()) {
            commonFunctions.m2508assert(this.firstKey == this.map.getFirstKey$runtime_release());
            commonFunctions.m2508assert(this.lastKey == this.map.getLastKey$runtime_release());
    ...
    public V put(K k, V v) {
        ...
        if (linkedValue != null) {
            ...
            commonFunctions.m2508assert(!linkedValue2.getHasNext());
