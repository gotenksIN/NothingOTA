...
final class ParentSizeModifier extends InspectorValueInfo implements LayoutModifier {
    ...
    public MeasureResult mo25measure3p2s80s(MeasureScope measure, Measurable measurable, long j) {
        ...
        int m5008getMinWidthimpl = roundToInt != Integer.MAX_VALUE ? roundToInt : Constraints.m5008getMinWidthimpl(j);
        int m5007getMinHeightimpl = roundToInt2 != Integer.MAX_VALUE ? roundToInt2 : Constraints.m5007getMinHeightimpl(j);
        if (roundToInt == Integer.MAX_VALUE) {
            roundToInt = Constraints.m5006getMaxWidthimpl(j);
        }
        if (roundToInt2 == Integer.MAX_VALUE) {
            roundToInt2 = Constraints.m5005getMaxHeightimpl(j);
        }
        final Placeable mo4075measureBRTryo0 = measurable.mo4075measureBRTryo0(ConstraintsKt.Constraints(m5008getMinWidthimpl, roundToInt, m5007getMinHeightimpl, roundToInt2));
        return MeasureScope.layout$default(measure, mo4075measureBRTryo0.getWidth(), mo4075measureBRTryo0.getHeight(), null, new Function1<Placeable.PlacementScope, Unit>() { // from class: androidx.compose.foundation.lazy.ParentSizeModifier$measure$1
