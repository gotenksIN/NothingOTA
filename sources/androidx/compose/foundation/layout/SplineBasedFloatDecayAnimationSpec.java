...
final class SplineBasedFloatDecayAnimationSpec implements FloatDecayAnimationSpec {
    ...
    public float getValueFromNanos(long j, float f, float f2) {
        ...
        return f + (flingDistance(f2) * AndroidFlingSpline.FlingResult.m655getDistanceCoefficientimpl(AndroidFlingSpline.INSTANCE.m650flingPositionLfoxSSI(durationNanos > 0 ? ((float) j) / ((float) durationNanos) : 1.0f)));
    }
    ...
    public float getVelocityFromNanos(long j, float f, float f2) {
        long durationNanos = getDurationNanos(0.0f, f2);
        return ((AndroidFlingSpline.FlingResult.m656getVelocityCoefficientimpl(AndroidFlingSpline.INSTANCE.m650flingPositionLfoxSSI(durationNanos > 0 ? ((float) j) / ((float) durationNanos) : 1.0f)) * flingDistance(f2)) / ((float) durationNanos)) * 1.0E9f;
    }
}
