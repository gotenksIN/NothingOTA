...
public final class MultiWidgetSelectionDelegate implements Selectable {
    ...
    private final synchronized int getLastVisibleOffset(TextLayoutResult textLayoutResult) {
        ...
        if (this._previousTextLayoutResult != textLayoutResult) {
            if (textLayoutResult.getDidOverflowHeight() && !textLayoutResult.getMultiParagraph().getDidExceedMaxLines()) {
                lineCount = RangesKt.coerceAtMost(textLayoutResult.getLineForVerticalPosition(IntSize.m5209getHeightimpl(textLayoutResult.m4560getSizeYbymL2g())), textLayoutResult.getLineCount() - 1);
                while (textLayoutResult.getLineTop(lineCount) >= IntSize.m5209getHeightimpl(textLayoutResult.m4560getSizeYbymL2g())) {
                    lineCount--;
                }
    ...
    public Pair<Selection, Boolean> mo809updateSelectionqCDeeow(long j, long j2, Offset offset, boolean z, LayoutCoordinates containerLayoutCoordinates, SelectionAdjustment adjustment, Selection selection) {
        ...
        if (layoutCoordinates != null && (invoke = this.layoutResultCallback.invoke()) != null) {
            long mo4081localPositionOfR5De75A = containerLayoutCoordinates.mo4081localPositionOfR5De75A(layoutCoordinates, Offset.Companion.m2334getZeroF1C5BW0());
            return MultiWidgetSelectionDelegateKt.m813getTextSelectionInfoyM0VcXU(invoke, Offset.m2322minusMKHz9U(j, mo4081localPositionOfR5De75A), Offset.m2322minusMKHz9U(j2, mo4081localPositionOfR5De75A), offset != null ? Offset.m2307boximpl(Offset.m2322minusMKHz9U(offset.m2328unboximpl(), mo4081localPositionOfR5De75A)) : null, getSelectableId(), adjustment, selection, z);
        }
    ...
    public Selection getSelectAllSelection() {
        ...
        return MultiWidgetSelectionDelegateKt.m810access$getAssembledSelectionInfovJH6DeI(TextRangeKt.TextRange(0, invoke.getLayoutInput().getText().length()), false, getSelectableId(), invoke);
    }

    @Override // androidx.compose.foundation.text.selection.Selectable
    /* renamed from: getHandlePosition-dBAh8RU  reason: not valid java name */
    public long mo807getHandlePositiondBAh8RU(Selection selection, boolean z) {
        TextLayoutResult invoke;
    ...
    public long mo808getRangeOfLineContainingjx7JFs(int i) {
        TextLayoutResult invoke = this.layoutResultCallback.invoke();
        if (invoke == null) {
