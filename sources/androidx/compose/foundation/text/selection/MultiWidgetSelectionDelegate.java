...
public final class MultiWidgetSelectionDelegate implements Selectable {
    ...
    private final synchronized int getLastVisibleOffset(TextLayoutResult textLayoutResult) {
        ...
        if (this._previousTextLayoutResult != textLayoutResult) {
            if (textLayoutResult.getDidOverflowHeight() && !textLayoutResult.getMultiParagraph().getDidExceedMaxLines()) {
                lineCount = RangesKt.coerceAtMost(textLayoutResult.getLineForVerticalPosition(IntSize.m5510getHeightimpl(textLayoutResult.m4861getSizeYbymL2g())), textLayoutResult.getLineCount() - 1);
                while (textLayoutResult.getLineTop(lineCount) >= IntSize.m5510getHeightimpl(textLayoutResult.m4861getSizeYbymL2g())) {
                    lineCount--;
                }
    ...
    public Tuples<Selection, Boolean> mo1110updateSelectionqCDeeow(long j, long j2, Offset offset, boolean z, LayoutCoordinates containerLayoutCoordinates, SelectionAdjustment adjustment, Selection selection) {
        ...
        if (layoutCoordinates != null && (invoke = this.layoutResultCallback.invoke()) != null) {
            long mo4382localPositionOfR5De75A = containerLayoutCoordinates.mo4382localPositionOfR5De75A(layoutCoordinates, Offset.Companion.m2635getZeroF1C5BW0());
            return MultiWidgetSelectionDelegateKt.m1114getTextSelectionInfoyM0VcXU(invoke, Offset.m2623minusMKHz9U(j, mo4382localPositionOfR5De75A), Offset.m2623minusMKHz9U(j2, mo4382localPositionOfR5De75A), offset != null ? Offset.m2608boximpl(Offset.m2623minusMKHz9U(offset.m2629unboximpl(), mo4382localPositionOfR5De75A)) : null, getSelectableId(), adjustment, selection, z);
        }
    ...
    public Selection getSelectAllSelection() {
        ...
        return MultiWidgetSelectionDelegateKt.m1111access$getAssembledSelectionInfovJH6DeI(TextRangeKt.TextRange(0, invoke.getLayoutInput().getText().length()), false, getSelectableId(), invoke);
    }

    @Override // androidx.compose.foundation.text.selection.Selectable
    /* renamed from: getHandlePosition-dBAh8RU  reason: not valid java name */
    public long mo1108getHandlePositiondBAh8RU(Selection selection, boolean z) {
        TextLayoutResult invoke;
        ...
        if (getLayoutCoordinates() != null && (invoke = this.layoutResultCallback.invoke()) != null) {
            return TextSelectionDelegate.getSelectionHandleCoordinates(invoke, RangesKt.coerceIn((z ? selection.getStart() : selection.getEnd()).getOffset(), 0, getLastVisibleOffset(invoke)), z, selection.getHandlesCrossed());
        }
        return Offset.Companion.m2635getZeroF1C5BW0();
    }
    ...
    public long mo1109getRangeOfLineContainingjx7JFs(int i) {
        TextLayoutResult invoke = this.layoutResultCallback.invoke();
        if (invoke == null) {
