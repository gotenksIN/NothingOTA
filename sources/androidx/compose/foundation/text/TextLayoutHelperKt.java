...
public final class TextLayoutHelperKt {
    ...
    public static final boolean m789canReuse7_7YC6M(TextLayoutResult canReuse, AnnotatedString text, TextStyle style, List<AnnotatedString.Range<Placeholder>> placeholders, int i, boolean z, int i2, Density density, LayoutDirection layoutDirection, FontFamily.Resolver fontFamilyResolver, long j) {
        ...
        if (!canReuse.getMultiParagraph().getIntrinsics().getHasStaleResolvedFonts() && Intrinsics.areEqual(layoutInput.getText(), text) && layoutInput.getStyle().hasSameLayoutAffectingAttributes(style) && Intrinsics.areEqual(layoutInput.getPlaceholders(), placeholders) && layoutInput.getMaxLines() == i && layoutInput.getSoftWrap() == z && TextOverflow.m4984equalsimpl0(layoutInput.m4556getOverflowgIe3tQ8(), i2) && Intrinsics.areEqual(layoutInput.getDensity(), density) && layoutInput.getLayoutDirection() == layoutDirection && Intrinsics.areEqual(layoutInput.getFontFamilyResolver(), fontFamilyResolver) && Constraints.m5008getMinWidthimpl(j) == Constraints.m5008getMinWidthimpl(layoutInput.m4555getConstraintsmsEJaDk())) {
            if (z || TextOverflow.m4984equalsimpl0(i2, TextOverflow.Companion.m4992getEllipsisgIe3tQ8())) {
                return Constraints.m5006getMaxWidthimpl(j) == Constraints.m5006getMaxWidthimpl(layoutInput.m4555getConstraintsmsEJaDk()) && Constraints.m5005getMaxHeightimpl(j) == Constraints.m5005getMaxHeightimpl(layoutInput.m4555getConstraintsmsEJaDk());
            }
            return true;
        }
        return false;
