...
public final class SemanticsNodeKt {
    ...
    public static final SemanticsModifierNode getOuterSemantics(LayoutNode layoutNode) {
        ...
        int m4277constructorimpl = NodeKind.m4277constructorimpl(8);
        if ((NodeChain.access$getAggregateChildKindSet(nodes$ui_release) & m4277constructorimpl) != 0) {
            node = nodes$ui_release.getHead$ui_release();
            while (node != null) {
                if ((node.getKindSet$ui_release() & m4277constructorimpl) != 0 && (node instanceof SemanticsModifierNode)) {
                    break;
                } else if ((node.getAggregateChildKindSet$ui_release() & m4277constructorimpl) == 0) {
                    break;
                } else {
    ...
    public static final SemanticsModifierNode getOuterMergingSemantics(LayoutNode layoutNode) {
        ...
        if ((NodeChain.access$getAggregateChildKindSet(nodes$ui_release) & m4277constructorimpl) != 0) {
            node = nodes$ui_release.getHead$ui_release();
            while (node != null) {
                if ((node.getKindSet$ui_release() & m4277constructorimpl) == 0 || !(node instanceof SemanticsModifierNode) || !((SemanticsModifierNode) node).getSemanticsConfiguration().isMergingSemanticsOfDescendants()) {
