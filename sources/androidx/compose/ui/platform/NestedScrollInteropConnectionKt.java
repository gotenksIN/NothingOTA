...
public final class NestedScrollInteropConnectionKt {
    ...
    public static final long m4402toOffsetUv8p0NA(int[] iArr, long j) {
        float coerceAtLeast;
        float coerceAtLeast2;
        if (Offset.m2318getXimpl(j) >= 0.0f) {
            coerceAtLeast = RangesKt.coerceAtMost(reverseAxis(iArr[0]), Offset.m2318getXimpl(j));
        } else {
            coerceAtLeast = RangesKt.coerceAtLeast(reverseAxis(iArr[0]), Offset.m2318getXimpl(j));
        }
        if (Offset.m2319getYimpl(j) >= 0.0f) {
            coerceAtLeast2 = RangesKt.coerceAtMost(reverseAxis(iArr[1]), Offset.m2319getYimpl(j));
        } else {
            coerceAtLeast2 = RangesKt.coerceAtLeast(reverseAxis(iArr[1]), Offset.m2319getYimpl(j));
        }
    ...
    public static final int m4403toViewTypeGyEprt8(int i) {
        return !NestedScrollSource.m3851equalsimpl0(i, NestedScrollSource.Companion.m3856getDragWNlRxjI()) ? 1 : 0;
    }

    /* JADX INFO: Access modifiers changed from: private */
    /* renamed from: getScrollAxes-k-4lQ0M  reason: not valid java name */
    public static final int m4401getScrollAxesk4lQ0M(long j) {
        int i = Math.abs(Offset.m2318getXimpl(j)) >= 0.5f ? 1 : 0;
        return Math.abs(Offset.m2319getYimpl(j)) >= 0.5f ? i | 2 : i;
    }
