...
public abstract class NodeCoordinator extends LookaheadCapablePlaceable implements Measurable, LayoutCoordinates, OwnerScope, Function1<Canvas, Unit> {
    ...
    private final Functions<Unit> invalidateParentLayer;
    ...
    private static final float[] tmpMatrix = Matrix.m3081constructorimpl$default(null, 1, null);
    private static final HitTestSource<PointerInputModifierNode> PointerInputSource = new HitTestSource<PointerInputModifierNode>() { // from class: androidx.compose.ui.node.NodeCoordinator$Companion$PointerInputSource$1
        @Override // androidx.compose.p002ui.node.NodeCoordinator.HitTestSource
        ...
        public void mo4573childHitTestYqVAtuI(LayoutNode layoutNode, long j, HitTestResult<PointerInputModifierNode> hitTestResult, boolean z, boolean z2) {
            Intrinsics.checkNotNullParameter(layoutNode, "layoutNode");
            Intrinsics.checkNotNullParameter(hitTestResult, "hitTestResult");
            layoutNode.m4505hitTestM_7yMNQ$ui_release(j, hitTestResult, z, z2);
        }

        @Override // androidx.compose.p002ui.node.NodeCoordinator.HitTestSource
        /* renamed from: entityType-OLwlOKw  reason: not valid java name */
        public int mo4574entityTypeOLwlOKw() {
            return NodeKind.m4578constructorimpl(16);
        }
    };
    ...
    public NodeCoordinator(LayoutNode layoutNode) {
        ...
        this.invalidateParentLayer = new Functions<Unit>() { // from class: androidx.compose.ui.node.NodeCoordinator$invalidateParentLayer$1
    ...
    public final /* synthetic */ <T> void m4571visitNodesaLcG6gQ(int i, Function1<? super T, Unit> block) {
        Intrinsics.checkNotNullParameter(block, "block");
        boolean m4586getIncludeSelfInTraversalH91voCI = NodeKindKt.m4586getIncludeSelfInTraversalH91voCI(i);
        Modifier.Node tail = getTail();
        if (!m4586getIncludeSelfInTraversalH91voCI && (tail = tail.getParent$ui_release()) == null) {
            return;
        }
        for (Modifier.Node headNode = headNode(m4586getIncludeSelfInTraversalH91voCI); headNode != null && (headNode.getAggregateChildKindSet$ui_release() & i) != 0; headNode = headNode.getChild$ui_release()) {
            if ((headNode.getKindSet$ui_release() & i) != 0) {
    ...
    public final boolean m4563hasNodeH91voCI(int i) {
        Modifier.Node headNode = headNode(NodeKindKt.m4586getIncludeSelfInTraversalH91voCI(i));
        return headNode != null && DelegatableNodeKt.m4478has64DMado(headNode, i);
    }

    /* renamed from: head-H91voCI  reason: not valid java name */
    public final /* synthetic */ <T> T m4564headH91voCI(int i) {
        boolean m4586getIncludeSelfInTraversalH91voCI = NodeKindKt.m4586getIncludeSelfInTraversalH91voCI(i);
        Modifier.Node tail = getTail();
        if (!m4586getIncludeSelfInTraversalH91voCI && (tail = tail.getParent$ui_release()) == null) {
            return null;
    ...
    public final <T> T m4565headUncheckedH91voCI(int i) {
        boolean m4586getIncludeSelfInTraversalH91voCI = NodeKindKt.m4586getIncludeSelfInTraversalH91voCI(i);
        Modifier.Node tail = getTail();
        if (!m4586getIncludeSelfInTraversalH91voCI && (tail = tail.getParent$ui_release()) == null) {
            return null;
    ...
    public final long mo4381getSizeYbymL2g() {
        return m4420getMeasuredSizeYbymL2g();
    }
    ...
    public void replace$ui_release() {
        mo4377placeAtf8xVGno(mo4527getPositionnOccac(), this.zIndex, this.layerBlock);
    }
    ...
    protected void onMeasureResultChanged(int i, int i2) {
        ...
        if (ownedLayer != null) {
            ownedLayer.mo4623resizeozmzZPI(IntSizeKt.IntSize(i, i2));
        } else {
            NodeCoordinator nodeCoordinator = this.wrappedBy;
            if (nodeCoordinator != null) {
        ...
        graphicsLayerScope.m3159setSizeuvyYCjk(IntSizeKt.m5521toSizeozmzZPI(m4420getMeasuredSizeYbymL2g()));
        int m4578constructorimpl = NodeKind.m4578constructorimpl(4);
        boolean m4586getIncludeSelfInTraversalH91voCI = NodeKindKt.m4586getIncludeSelfInTraversalH91voCI(m4578constructorimpl);
        Modifier.Node tail = getTail();
        if (!m4586getIncludeSelfInTraversalH91voCI && (tail = tail.getParent$ui_release()) == null) {
            return;
        }
        for (Modifier.Node headNode = headNode(m4586getIncludeSelfInTraversalH91voCI); headNode != null && (headNode.getAggregateChildKindSet$ui_release() & m4578constructorimpl) != 0; headNode = headNode.getChild$ui_release()) {
            if ((headNode.getKindSet$ui_release() & m4578constructorimpl) != 0 && (headNode instanceof DrawModifierNode)) {
    ...
    protected void m4569setPositiongyyYBs(long j) {
        this.position = j;
    }
    ...
    public Object getParentData() {
        ...
        Modifier.Node tail = getTail();
        if (getLayoutNode().getNodes$ui_release().m4541hasH91voCI$ui_release(NodeKind.m4578constructorimpl(64))) {
            ...
            for (Modifier.Node tail$ui_release = getLayoutNode().getNodes$ui_release().getTail$ui_release(); tail$ui_release != null; tail$ui_release = tail$ui_release.getParent$ui_release()) {
                if (tail$ui_release != tail) {
                    if (((NodeKind.m4578constructorimpl(64) & tail$ui_release.getKindSet$ui_release()) != 0) && (tail$ui_release instanceof ParentDataModifierNode)) {
    ...
    public final long m4561getLastMeasurementConstraintsmsEJaDk$ui_release() {
        return m4421getMeasurementConstraintsmsEJaDk();
    }

    /* renamed from: performingMeasure-K40F9xA  reason: not valid java name */
    protected final Placeable m4568performingMeasureK40F9xA(long j, Functions<? extends Placeable> block) {
        Intrinsics.checkNotNullParameter(block, "block");
        m4423setMeasurementConstraintsBRTryo0(j);
        Placeable invoke = block.invoke();
        OwnedLayer layer = getLayer();
        if (layer != null) {
            layer.mo4623resizeozmzZPI(m4420getMeasuredSizeYbymL2g());
        }
        return invoke;
    }
    ...
    public void mo4377placeAtf8xVGno(long j, float f, Function1<? super GraphicsLayerScope, Unit> function1) {
        ...
        if (!IntOffset.m5468equalsimpl0(mo4527getPositionnOccac(), j)) {
            m4569setPositiongyyYBs(j);
            getLayoutNode().getLayoutDelegate$ui_release().getMeasurePassDelegate$ui_release().notifyChildrenUsingCoordinatesWhilePlacing();
            OwnedLayer ownedLayer = this.layer;
            if (ownedLayer != null) {
                ownedLayer.mo4622movegyyYBs(j);
            } else {
                NodeCoordinator nodeCoordinator = this.wrappedBy;
                if (nodeCoordinator != null) {
                    nodeCoordinator.invalidateLayer();
    ...
    public final void draw(Canvas canvas) {
        ...
        float m5469getXimpl = IntOffset.m5469getXimpl(mo4527getPositionnOccac());
        float m5470getYimpl = IntOffset.m5470getYimpl(mo4527getPositionnOccac());
        canvas.translate(m5469getXimpl, m5470getYimpl);
    ...
    public final void onPlaced() {
        ...
        if (lookaheadDelegate != null) {
            int m4578constructorimpl = NodeKind.m4578constructorimpl(128);
            boolean m4586getIncludeSelfInTraversalH91voCI = NodeKindKt.m4586getIncludeSelfInTraversalH91voCI(m4578constructorimpl);
            Modifier.Node tail = getTail();
            if (m4586getIncludeSelfInTraversalH91voCI || (tail = tail.getParent$ui_release()) != null) {
                for (Modifier.Node headNode = headNode(m4586getIncludeSelfInTraversalH91voCI); headNode != null && (headNode.getAggregateChildKindSet$ui_release() & m4578constructorimpl) != 0; headNode = headNode.getChild$ui_release()) {
                    if ((headNode.getKindSet$ui_release() & m4578constructorimpl) != 0 && (headNode instanceof LayoutAwareModifierNode)) {
        ...
        int m4578constructorimpl2 = NodeKind.m4578constructorimpl(128);
        boolean m4586getIncludeSelfInTraversalH91voCI2 = NodeKindKt.m4586getIncludeSelfInTraversalH91voCI(m4578constructorimpl2);
        Modifier.Node tail2 = getTail();
        if (!m4586getIncludeSelfInTraversalH91voCI2 && (tail2 = tail2.getParent$ui_release()) == null) {
            return;
        }
        for (Modifier.Node headNode2 = headNode(m4586getIncludeSelfInTraversalH91voCI2); headNode2 != null && (headNode2.getAggregateChildKindSet$ui_release() & m4578constructorimpl2) != 0; headNode2 = headNode2.getChild$ui_release()) {
            if ((headNode2.getKindSet$ui_release() & m4578constructorimpl2) != 0 && (headNode2 instanceof LayoutAwareModifierNode)) {
    ...
    public void invoke2(final Canvas canvas) {
        ...
        if (getLayoutNode().isPlaced()) {
            getSnapshotObserver().observeReads$ui_release(this, onCommitAffectingLayer, new Functions<Unit>() { // from class: androidx.compose.ui.node.NodeCoordinator$invoke$1
                /* JADX INFO: Access modifiers changed from: package-private */
    ...
    private final void onLayerBlockUpdated(Function1<? super GraphicsLayerScope, Unit> function1, boolean z) {
        ...
        if (isAttached() && function1 != null) {
            ...
            createLayer.mo4623resizeozmzZPI(m4420getMeasuredSizeYbymL2g());
            createLayer.mo4622movegyyYBs(mo4527getPositionnOccac());
    ...
    public final void updateLayerParameters() {
        ...
        if (ownedLayer != null) {
            ...
            reusableGraphicsLayerScope.m3159setSizeuvyYCjk(IntSizeKt.m5521toSizeozmzZPI(mo4381getSizeYbymL2g()));
            getSnapshotObserver().observeReads$ui_release(this, onCommitAffectingLayerParams, new Functions<Unit>() { // from class: androidx.compose.ui.node.NodeCoordinator$updateLayerParameters$1
                /* JADX INFO: Access modifiers changed from: package-private */
                /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    ...
    public final long m4562getMinimumTouchTargetSizeNHjbRc() {
        return this.layerDensity.mo604toSizeXkaWNTQ(getLayoutNode().getViewConfiguration().mo4509getMinimumTouchTargetSizeMYxV2XQ());
    }

    /* JADX WARN: Multi-variable type inference failed */
    /* renamed from: hitTest-YqVAtuI  reason: not valid java name */
    public final <T extends DelegatableNode> void m4566hitTestYqVAtuI(HitTestSource<T> hitTestSource, long j, HitTestResult<T> hitTestResult, boolean z, boolean z2) {
        Intrinsics.checkNotNullParameter(hitTestSource, "hitTestSource");
        Intrinsics.checkNotNullParameter(hitTestResult, "hitTestResult");
        DelegatableNode delegatableNode = (DelegatableNode) m4565headUncheckedH91voCI(hitTestSource.mo4574entityTypeOLwlOKw());
        boolean z3 = true;
        if (!m4572withinLayerBoundsk4lQ0M(j)) {
            if (z) {
                float m4559distanceInMinimumTouchTargettz77jQw = m4559distanceInMinimumTouchTargettz77jQw(j, m4562getMinimumTouchTargetSizeNHjbRc());
                if (Float.isInfinite(m4559distanceInMinimumTouchTargettz77jQw) || Float.isNaN(m4559distanceInMinimumTouchTargettz77jQw)) {
                    z3 = false;
                }
                if (z3 && hitTestResult.isHitInMinimumTouchTargetBetter(m4559distanceInMinimumTouchTargettz77jQw, false)) {
                    m4553hitNearJHbHoSQ(delegatableNode, hitTestSource, j, hitTestResult, z, false, m4559distanceInMinimumTouchTargettz77jQw);
                }
            }
        } else if (delegatableNode == null) {
            mo4500hitTestChildYqVAtuI(hitTestSource, j, hitTestResult, z, z2);
        } else if (m4567isPointerInBoundsk4lQ0M(j)) {
            m4552hit1hIXUjU(delegatableNode, hitTestSource, j, hitTestResult, z, z2);
        } else {
            float m4559distanceInMinimumTouchTargettz77jQw2 = !z ? Float.POSITIVE_INFINITY : m4559distanceInMinimumTouchTargettz77jQw(j, m4562getMinimumTouchTargetSizeNHjbRc());
            if (Float.isInfinite(m4559distanceInMinimumTouchTargettz77jQw2) || Float.isNaN(m4559distanceInMinimumTouchTargettz77jQw2)) {
                z3 = false;
            }
            if (z3 && hitTestResult.isHitInMinimumTouchTargetBetter(m4559distanceInMinimumTouchTargettz77jQw2, z2)) {
                m4553hitNearJHbHoSQ(delegatableNode, hitTestSource, j, hitTestResult, z, z2, m4559distanceInMinimumTouchTargettz77jQw2);
            } else {
                m4555speculativeHitJHbHoSQ(delegatableNode, hitTestSource, j, hitTestResult, z, z2, m4559distanceInMinimumTouchTargettz77jQw2);
            }
        }
    }
    ...
    public final <T extends DelegatableNode> void m4552hit1hIXUjU(final T t, final HitTestSource<T> hitTestSource, final long j, final HitTestResult<T> hitTestResult, final boolean z, final boolean z2) {
        if (t == null) {
            mo4500hitTestChildYqVAtuI(hitTestSource, j, hitTestResult, z, z2);
        } else {
            hitTestResult.hit(t, z2, new Functions<Unit>() { // from class: androidx.compose.ui.node.NodeCoordinator$hit$1
                /* JADX INFO: Access modifiers changed from: package-private */
                /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
                ...
                /* renamed from: invoke  reason: avoid collision after fix types in other method */
                public final void invoke2() {
                    Object m4576nextUncheckedUntilhw7D004;
                    NodeCoordinator nodeCoordinator = NodeCoordinator.this;
                    m4576nextUncheckedUntilhw7D004 = NodeCoordinatorKt.m4576nextUncheckedUntilhw7D004(t, hitTestSource.mo4574entityTypeOLwlOKw(), NodeKind.m4578constructorimpl(2));
                    nodeCoordinator.m4552hit1hIXUjU((DelegatableNode) m4576nextUncheckedUntilhw7D004, hitTestSource, j, hitTestResult, z, z2);
                }
            });
        }
    }
    ...
    public final <T extends DelegatableNode> void m4553hitNearJHbHoSQ(final T t, final HitTestSource<T> hitTestSource, final long j, final HitTestResult<T> hitTestResult, final boolean z, final boolean z2, final float f) {
        if (t == null) {
            mo4500hitTestChildYqVAtuI(hitTestSource, j, hitTestResult, z, z2);
        } else {
            hitTestResult.hitInMinimumTouchTarget(t, f, z2, new Functions<Unit>() { // from class: androidx.compose.ui.node.NodeCoordinator$hitNear$1
                /* JADX INFO: Access modifiers changed from: package-private */
                /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
                ...
                public final void invoke2() {
                    ...
                    NodeCoordinator nodeCoordinator = NodeCoordinator.this;
                    m4576nextUncheckedUntilhw7D004 = NodeCoordinatorKt.m4576nextUncheckedUntilhw7D004(t, hitTestSource.mo4574entityTypeOLwlOKw(), NodeKind.m4578constructorimpl(2));
                    nodeCoordinator.m4553hitNearJHbHoSQ((DelegatableNode) m4576nextUncheckedUntilhw7D004, hitTestSource, j, hitTestResult, z, z2, f);
                }
            });
        }
    }
    ...
    public final <T extends DelegatableNode> void m4555speculativeHitJHbHoSQ(final T t, final HitTestSource<T> hitTestSource, final long j, final HitTestResult<T> hitTestResult, final boolean z, final boolean z2, final float f) {
        if (t == null) {
            mo4500hitTestChildYqVAtuI(hitTestSource, j, hitTestResult, z, z2);
        } else if (hitTestSource.interceptOutOfBoundsChildEvents(t)) {
            hitTestResult.speculativeHit(t, f, z2, new Functions<Unit>() { // from class: androidx.compose.ui.node.NodeCoordinator$speculativeHit$1
                /* JADX INFO: Access modifiers changed from: package-private */
                /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
                /* JADX WARN: Incorrect types in method signature: (Landroidx/compose/ui/node/NodeCoordinator;TT;Landroidx/compose/ui/node/NodeCoordinator$HitTestSource<TT;>;JLandroidx/compose/ui/node/HitTestResult<TT;>;ZZF)V */
                {
                ...
                public final void invoke2() {
                    ...
                    NodeCoordinator nodeCoordinator = NodeCoordinator.this;
                    m4576nextUncheckedUntilhw7D004 = NodeCoordinatorKt.m4576nextUncheckedUntilhw7D004(t, hitTestSource.mo4574entityTypeOLwlOKw(), NodeKind.m4578constructorimpl(2));
                    nodeCoordinator.m4555speculativeHitJHbHoSQ((DelegatableNode) m4576nextUncheckedUntilhw7D004, hitTestSource, j, hitTestResult, z, z2, f);
                }
            });
        } else {
            m4555speculativeHitJHbHoSQ((DelegatableNode) NodeCoordinatorKt.m4575access$nextUncheckedUntilhw7D004(t, hitTestSource.mo4574entityTypeOLwlOKw(), NodeKind.m4578constructorimpl(2)), hitTestSource, j, hitTestResult, z, z2, f);
        }
    }

    /* renamed from: hitTestChild-YqVAtuI */
    public <T extends DelegatableNode> void mo4500hitTestChildYqVAtuI(HitTestSource<T> hitTestSource, long j, HitTestResult<T> hitTestResult, boolean z, boolean z2) {
        Intrinsics.checkNotNullParameter(hitTestSource, "hitTestSource");
        Intrinsics.checkNotNullParameter(hitTestResult, "hitTestResult");
        NodeCoordinator nodeCoordinator = this.wrapped;
        if (nodeCoordinator != null) {
            nodeCoordinator.m4566hitTestYqVAtuI(hitTestSource, nodeCoordinator.m4560fromParentPositionMKHz9U(j), hitTestResult, z, z2);
        }
    }
    ...
    public final Rect touchBoundsInRoot() {
        ...
        long m4558calculateMinimumTouchTargetPaddingE7KxVPU = m4558calculateMinimumTouchTargetPaddingE7KxVPU(m4562getMinimumTouchTargetSizeNHjbRc());
        rectCache.setLeft(-Size.m2688getWidthimpl(m4558calculateMinimumTouchTargetPaddingE7KxVPU));
        rectCache.setTop(-Size.m2685getHeightimpl(m4558calculateMinimumTouchTargetPaddingE7KxVPU));
        rectCache.setRight(getMeasuredWidth() + Size.m2688getWidthimpl(m4558calculateMinimumTouchTargetPaddingE7KxVPU));
        rectCache.setBottom(getMeasuredHeight() + Size.m2685getHeightimpl(m4558calculateMinimumTouchTargetPaddingE7KxVPU));
    ...
    public long mo4386windowToLocalMKHz9U(long j) {
        ...
        LayoutCoordinates findRootCoordinates = LayoutCoordinatesKt.findRootCoordinates(this);
        return mo4382localPositionOfR5De75A(findRootCoordinates, Offset.m2623minusMKHz9U(LayoutNodeKt.requireOwner(getLayoutNode()).mo4626calculateLocalPositionMKHz9U(j), LayoutCoordinatesKt.positionInRoot(findRootCoordinates)));
    }

    @Override // androidx.compose.p002ui.layout.LayoutCoordinates
    /* renamed from: localToWindow-MK-Hz9U */
    public long mo4384localToWindowMKHz9U(long j) {
        return LayoutNodeKt.requireOwner(getLayoutNode()).mo4627calculatePositionInWindowMKHz9U(mo4383localToRootMKHz9U(j));
    }
    ...
    public long mo4382localPositionOfR5De75A(LayoutCoordinates sourceCoordinates, long j) {
        ...
        while (coordinator != findCommonAncestor$ui_release) {
            j = coordinator.m4570toParentPositionMKHz9U(j);
            coordinator = coordinator.wrappedBy;
            Intrinsics.checkNotNull(coordinator);
        }
        return m4551ancestorToLocalR5De75A(findCommonAncestor$ui_release, j);
    }
    ...
    public void mo4385transformFromEL8BTi8(LayoutCoordinates sourceCoordinates, float[] matrix) {
        ...
        NodeCoordinator coordinator = toCoordinator(sourceCoordinates);
        NodeCoordinator findCommonAncestor$ui_release = findCommonAncestor$ui_release(coordinator);
        Matrix.m3090resetimpl(matrix);
        coordinator.m4557transformToAncestorEL8BTi8(findCommonAncestor$ui_release, matrix);
        m4556transformFromAncestorEL8BTi8(findCommonAncestor$ui_release, matrix);
    }

    /* renamed from: transformToAncestor-EL8BTi8  reason: not valid java name */
    private final void m4557transformToAncestorEL8BTi8(NodeCoordinator nodeCoordinator, float[] fArr) {
        NodeCoordinator nodeCoordinator2 = this;
        while (!Intrinsics.areEqual(nodeCoordinator2, nodeCoordinator)) {
            OwnedLayer ownedLayer = nodeCoordinator2.layer;
            if (ownedLayer != null) {
                ownedLayer.mo4624transform58bKbWc(fArr);
            }
            long mo4527getPositionnOccac = nodeCoordinator2.mo4527getPositionnOccac();
            if (!IntOffset.m5468equalsimpl0(mo4527getPositionnOccac, IntOffset.Companion.m5479getZeronOccac())) {
                float[] fArr2 = tmpMatrix;
                Matrix.m3090resetimpl(fArr2);
                Matrix.m3101translateimpl$default(fArr2, IntOffset.m5469getXimpl(mo4527getPositionnOccac), IntOffset.m5470getYimpl(mo4527getPositionnOccac), 0.0f, 4, null);
                Matrix.m3098timesAssign58bKbWc(fArr, fArr2);
            }
    ...
    private final void m4556transformFromAncestorEL8BTi8(NodeCoordinator nodeCoordinator, float[] fArr) {
        if (Intrinsics.areEqual(nodeCoordinator, this)) {
            return;
        }
        NodeCoordinator nodeCoordinator2 = this.wrappedBy;
        Intrinsics.checkNotNull(nodeCoordinator2);
        nodeCoordinator2.m4556transformFromAncestorEL8BTi8(nodeCoordinator, fArr);
        if (!IntOffset.m5468equalsimpl0(mo4527getPositionnOccac(), IntOffset.Companion.m5479getZeronOccac())) {
            float[] fArr2 = tmpMatrix;
            Matrix.m3090resetimpl(fArr2);
            Matrix.m3101translateimpl$default(fArr2, -IntOffset.m5469getXimpl(mo4527getPositionnOccac()), -IntOffset.m5470getYimpl(mo4527getPositionnOccac()), 0.0f, 4, null);
        ...
        OwnedLayer ownedLayer = this.layer;
        if (ownedLayer != null) {
            ownedLayer.mo4619inverseTransform58bKbWc(fArr);
        }
    }
    ...
    public Rect localBoundingBoxOf(LayoutCoordinates sourceCoordinates, boolean z) {
        ...
        rectCache.setRight(IntSize.m5511getWidthimpl(sourceCoordinates.mo4381getSizeYbymL2g()));
        rectCache.setBottom(IntSize.m5510getHeightimpl(sourceCoordinates.mo4381getSizeYbymL2g()));
        while (coordinator != findCommonAncestor$ui_release) {
    ...
    private final long m4551ancestorToLocalR5De75A(NodeCoordinator nodeCoordinator, long j) {
        if (nodeCoordinator == this) {
            return j;
        }
        NodeCoordinator nodeCoordinator2 = this.wrappedBy;
        if (nodeCoordinator2 == null || Intrinsics.areEqual(nodeCoordinator, nodeCoordinator2)) {
            return m4560fromParentPositionMKHz9U(j);
        }
        return m4560fromParentPositionMKHz9U(nodeCoordinator2.m4551ancestorToLocalR5De75A(nodeCoordinator, j));
    }
    ...
    public long mo4383localToRootMKHz9U(long j) {
        if (isAttached()) {
            for (NodeCoordinator nodeCoordinator = this; nodeCoordinator != null; nodeCoordinator = nodeCoordinator.wrappedBy) {
                j = nodeCoordinator.m4570toParentPositionMKHz9U(j);
            }
            return j;
        }
        throw new IllegalStateException(ExpectAttachedLayoutCoordinates.toString());
    ...
    public long m4570toParentPositionMKHz9U(long j) {
        OwnedLayer ownedLayer = this.layer;
        if (ownedLayer != null) {
            j = ownedLayer.mo4621mapOffset8S9VItk(j, false);
        }
        return IntOffsetKt.m5483plusNvtHpc(j, mo4527getPositionnOccac());
    }

    /* renamed from: fromParentPosition-MK-Hz9U  reason: not valid java name */
    public long m4560fromParentPositionMKHz9U(long j) {
        long m5481minusNvtHpc = IntOffsetKt.m5481minusNvtHpc(j, mo4527getPositionnOccac());
        OwnedLayer ownedLayer = this.layer;
        return ownedLayer != null ? ownedLayer.mo4621mapOffset8S9VItk(m5481minusNvtHpc, true) : m5481minusNvtHpc;
    }
    ...
    public final void drawBorder(Canvas canvas, Paint paint) {
        ...
        canvas.drawRect(new Rect(0.5f, 0.5f, IntSize.m5511getWidthimpl(m4420getMeasuredSizeYbymL2g()) - 0.5f, IntSize.m5510getHeightimpl(m4420getMeasuredSizeYbymL2g()) - 0.5f), paint);
    }
    ...
    public final void rectInParent$ui_release(MutableRect bounds, boolean z, boolean z2) {
        ...
        if (ownedLayer != null) {
            if (this.isClipping) {
                if (z2) {
                    long m4562getMinimumTouchTargetSizeNHjbRc = m4562getMinimumTouchTargetSizeNHjbRc();
                    float m2688getWidthimpl = Size.m2688getWidthimpl(m4562getMinimumTouchTargetSizeNHjbRc) / 2.0f;
                    float m2685getHeightimpl = Size.m2685getHeightimpl(m4562getMinimumTouchTargetSizeNHjbRc) / 2.0f;
                    bounds.intersect(-m2688getWidthimpl, -m2685getHeightimpl, IntSize.m5511getWidthimpl(mo4381getSizeYbymL2g()) + m2688getWidthimpl, IntSize.m5510getHeightimpl(mo4381getSizeYbymL2g()) + m2685getHeightimpl);
                } else if (z) {
                    bounds.intersect(0.0f, 0.0f, IntSize.m5511getWidthimpl(mo4381getSizeYbymL2g()), IntSize.m5510getHeightimpl(mo4381getSizeYbymL2g()));
                }
                if (bounds.isEmpty()) {
                    return;
                }
        ...
        float m5469getXimpl = IntOffset.m5469getXimpl(mo4527getPositionnOccac());
        bounds.setLeft(bounds.getLeft() + m5469getXimpl);
        bounds.setRight(bounds.getRight() + m5469getXimpl);
        float m5470getYimpl = IntOffset.m5470getYimpl(mo4527getPositionnOccac());
        bounds.setTop(bounds.getTop() + m5470getYimpl);
        bounds.setBottom(bounds.getBottom() + m5470getYimpl);
    }

    private final void fromParentRect(MutableRect mutableRect, boolean z) {
        float m5469getXimpl = IntOffset.m5469getXimpl(mo4527getPositionnOccac());
        mutableRect.setLeft(mutableRect.getLeft() - m5469getXimpl);
        mutableRect.setRight(mutableRect.getRight() - m5469getXimpl);
        float m5470getYimpl = IntOffset.m5470getYimpl(mo4527getPositionnOccac());
        mutableRect.setTop(mutableRect.getTop() - m5470getYimpl);
        mutableRect.setBottom(mutableRect.getBottom() - m5470getYimpl);
        OwnedLayer ownedLayer = this.layer;
        if (ownedLayer != null) {
            ownedLayer.mapBounds(mutableRect, true);
            if (this.isClipping && z) {
                mutableRect.intersect(0.0f, 0.0f, IntSize.m5511getWidthimpl(mo4381getSizeYbymL2g()), IntSize.m5510getHeightimpl(mo4381getSizeYbymL2g()));
                mutableRect.isEmpty();
            }
        }
    }
    ...
    public final boolean m4572withinLayerBoundsk4lQ0M(long j) {
        if (OffsetKt.m2636isFinitek4lQ0M(j)) {
            OwnedLayer ownedLayer = this.layer;
            return ownedLayer == null || !this.isClipping || ownedLayer.mo4620isInLayerk4lQ0M(j);
        }
        return false;
    }

    /* renamed from: isPointerInBounds-k-4lQ0M  reason: not valid java name */
    protected final boolean m4567isPointerInBoundsk4lQ0M(long j) {
        float m2619getXimpl = Offset.m2619getXimpl(j);
        float m2620getYimpl = Offset.m2620getYimpl(j);
        return m2619getXimpl >= 0.0f && m2620getYimpl >= 0.0f && m2619getXimpl < ((float) getMeasuredWidth()) && m2620getYimpl < ((float) getMeasuredHeight());
    }
    ...
    static /* synthetic */ Object propagateRelocationRequest$suspendImpl(NodeCoordinator nodeCoordinator, Rect rect, Continuation<? super Unit> continuation) {
        ...
        return (nodeCoordinator2 != null && (propagateRelocationRequest = nodeCoordinator2.propagateRelocationRequest(rect.m2656translatek4lQ0M(nodeCoordinator2.localBoundingBoxOf(nodeCoordinator, false).m2654getTopLeftF1C5BW0()), continuation)) == IntrinsicsKt.getCOROUTINE_SUSPENDED()) ? propagateRelocationRequest : Unit.INSTANCE;
    }
    ...
    public final NodeCoordinator findCommonAncestor$ui_release(NodeCoordinator other) {
        ...
        if (layoutNode == layoutNode2) {
            ...
            Modifier.Node tail2 = getTail();
            int m4578constructorimpl = NodeKind.m4578constructorimpl(2);
            ...
            for (Modifier.Node parent$ui_release = tail2.getNode().getParent$ui_release(); parent$ui_release != null; parent$ui_release = parent$ui_release.getParent$ui_release()) {
                if ((parent$ui_release.getKindSet$ui_release() & m4578constructorimpl) != 0 && parent$ui_release == tail) {
                    return other;
                }
            }
            return this;
    ...
    private final long m4554offsetFromEdgeMKHz9U(long j) {
        float m2619getXimpl = Offset.m2619getXimpl(j);
        float max = Math.max(0.0f, m2619getXimpl < 0.0f ? -m2619getXimpl : m2619getXimpl - getMeasuredWidth());
        float m2620getYimpl = Offset.m2620getYimpl(j);
        return OffsetKt.Offset(max, Math.max(0.0f, m2620getYimpl < 0.0f ? -m2620getYimpl : m2620getYimpl - getMeasuredHeight()));
    }

    /* renamed from: calculateMinimumTouchTargetPadding-E7KxVPU  reason: not valid java name */
    protected final long m4558calculateMinimumTouchTargetPaddingE7KxVPU(long j) {
        return SizeKt.Size(Math.max(0.0f, (Size.m2688getWidthimpl(j) - getMeasuredWidth()) / 2.0f), Math.max(0.0f, (Size.m2685getHeightimpl(j) - getMeasuredHeight()) / 2.0f));
    }

    /* JADX INFO: Access modifiers changed from: protected */
    /* renamed from: distanceInMinimumTouchTarget-tz77jQw  reason: not valid java name */
    public final float m4559distanceInMinimumTouchTargettz77jQw(long j, long j2) {
        if (getMeasuredWidth() < Size.m2688getWidthimpl(j2) || getMeasuredHeight() < Size.m2685getHeightimpl(j2)) {
            long m4558calculateMinimumTouchTargetPaddingE7KxVPU = m4558calculateMinimumTouchTargetPaddingE7KxVPU(j2);
            float m2688getWidthimpl = Size.m2688getWidthimpl(m4558calculateMinimumTouchTargetPaddingE7KxVPU);
            float m2685getHeightimpl = Size.m2685getHeightimpl(m4558calculateMinimumTouchTargetPaddingE7KxVPU);
            long m4554offsetFromEdgeMKHz9U = m4554offsetFromEdgeMKHz9U(j);
            if ((m2688getWidthimpl > 0.0f || m2685getHeightimpl > 0.0f) && Offset.m2619getXimpl(m4554offsetFromEdgeMKHz9U) <= m2688getWidthimpl && Offset.m2620getYimpl(m4554offsetFromEdgeMKHz9U) <= m2685getHeightimpl) {
                return Offset.m2618getDistanceSquaredimpl(m4554offsetFromEdgeMKHz9U);
            }
    ...
    /* compiled from: NodeCoordinator.kt */
    @Metadata(m41d1 = {"\u0000J\n\u0002\u0018\u0002\n\u0002\u0010\u0000\n\u0002\b\u0002\n\u0002\u0010\u000e\n\u0000\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0002\b\u0004\n\u0002\u0018\u0002\n\u0002\b\u0003\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0002\u0010\u0002\n\u0002\b\u0002\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\b\u0002\b\u0080\u0003\u0018\u00002\u00020\u0001B\u0007\b\u0002¢\u0006\u0002\u0010\u0002R\u000e\u0010\u0003\u001a\u00020\u0004X\u0086T¢\u0006\u0002\n\u0000R\u001d\u0010\u0005\u001a\b\u0012\u0004\u0012\u00020\u00070\u0006¢\u0006\u000e\n\u0000\u0012\u0004\b\b\u0010\u0002\u001a\u0004\b\t\u0010\nR\u0017\u0010\u000b\u001a\b\u0012\u0004\u0012\u00020\f0\u0006¢\u0006\b\n\u0000\u001a\u0004\b\r\u0010\nR\u000e\u0010\u000e\u001a\u00020\u0004X\u0086T¢\u0006\u0002\n\u0000R\u000e\u0010\u000f\u001a\u00020\u0010X\u0082\u0004¢\u0006\u0002\n\u0000R\u001a\u0010\u0011\u001a\u000e\u0012\u0004\u0012\u00020\u0013\u0012\u0004\u0012\u00020\u00140\u0012X\u0082\u0004¢\u0006\u0002\n\u0000R\u001a\u0010\u0015\u001a\u000e\u0012\u0004\u0012\u00020\u0013\u0012\u0004\u0012\u00020\u00140\u0012X\u0082\u0004¢\u0006\u0002\n\u0000R\u000e\u0010\u0016\u001a\u00020\u0017X\u0082\u0004¢\u0006\u0002\n\u0000R\u0019\u0010\u0018\u001a\u00020\u0019X\u0082\u0004ø\u0001\u0000ø\u0001\u0001ø\u0001\u0002¢\u0006\u0004\n\u0002\u0010\u001a\u0082\u0002\u000f\n\u0002\b\u0019\n\u0005\b¡\u001e0\u0001\n\u0002\b!¨\u0006\u001b"}, m40d2 = {"Landroidx/compose/ui/node/NodeCoordinator$Companion;", "", "()V", "ExpectAttachedLayoutCoordinates", "", "PointerInputSource", "Landroidx/compose/ui/node/NodeCoordinator$HitTestSource;", "Landroidx/compose/ui/node/PointerInputModifierNode;", "getPointerInputSource$annotations", "getPointerInputSource", "()Landroidx/compose/ui/node/NodeCoordinator$HitTestSource;", "SemanticsSource", "Landroidx/compose/ui/node/SemanticsModifierNode;", "getSemanticsSource", "UnmeasuredError", "graphicsLayerScope", "Landroidx/compose/ui/graphics/ReusableGraphicsLayerScope;", "onCommitAffectingLayer", "Lkotlin/Function1;", "Landroidx/compose/ui/node/NodeCoordinator;", "", "onCommitAffectingLayerParams", "tmpLayerPositionalProperties", "Landroidx/compose/ui/node/LayerPositionalProperties;", "tmpMatrix", "Landroidx/compose/ui/graphics/Matrix;", "[F", "ui_release"}, m39k = 1, m38mv = {1, 8, 0}, m36xi = 48)
    ...
    public final void onMeasured() {
        ...
        if (m4563hasNodeH91voCI(NodeKind.m4578constructorimpl(128))) {
            Snapshot createNonObservableSnapshot = Snapshot.Companion.createNonObservableSnapshot();
            try {
                Snapshot makeCurrent = createNonObservableSnapshot.makeCurrent();
                int m4578constructorimpl = NodeKind.m4578constructorimpl(128);
                boolean m4586getIncludeSelfInTraversalH91voCI = NodeKindKt.m4586getIncludeSelfInTraversalH91voCI(m4578constructorimpl);
                if (m4586getIncludeSelfInTraversalH91voCI) {
                ...
                for (Modifier.Node headNode = headNode(m4586getIncludeSelfInTraversalH91voCI); headNode != null && (headNode.getAggregateChildKindSet$ui_release() & m4578constructorimpl) != 0; headNode = headNode.getChild$ui_release()) {
                    if ((headNode.getKindSet$ui_release() & m4578constructorimpl) != 0 && (headNode instanceof LayoutAwareModifierNode)) {
                        ((LayoutAwareModifierNode) headNode).mo4464onRemeasuredozmzZPI(m4420getMeasuredSizeYbymL2g());
                    }
    ...
    public final void drawContainedDrawModifiers(Canvas canvas) {
        ...
        Modifier.Node tail = getTail();
        if (m4586getIncludeSelfInTraversalH91voCI || (tail = tail.getParent$ui_release()) != null) {
            Modifier.Node headNode = headNode(m4586getIncludeSelfInTraversalH91voCI);
            while (true) {
                if (headNode != null && (headNode.getAggregateChildKindSet$ui_release() & m4578constructorimpl) != 0) {
                    if ((headNode.getKindSet$ui_release() & m4578constructorimpl) == 0) {
                        if (headNode == tail) {
                            break;
                        }
        ...
        if (drawModifierNode == null) {
            ...
            getLayoutNode().getMDrawScope$ui_release().m4515drawx_KDEd0$ui_release(canvas, IntSizeKt.m5521toSizeozmzZPI(mo4381getSizeYbymL2g()), this, drawModifierNode);
        }
    }

    public final boolean shouldSharePointerInputWithSiblings() {
        Modifier.Node headNode = headNode(NodeKindKt.m4586getIncludeSelfInTraversalH91voCI(NodeKind.m4578constructorimpl(16)));
        if (headNode == null) {
            return false;
        }
        Modifier.Node node = headNode;
        int m4578constructorimpl = NodeKind.m4578constructorimpl(16);
        if (!node.getNode().isAttached()) {
            throw new IllegalStateException("Check failed.".toString());
        }
        Modifier.Node node2 = node.getNode();
        if ((node2.getAggregateChildKindSet$ui_release() & m4578constructorimpl) != 0) {
            for (Modifier.Node child$ui_release = node2.getChild$ui_release(); child$ui_release != null; child$ui_release = child$ui_release.getChild$ui_release()) {
                if ((child$ui_release.getKindSet$ui_release() & m4578constructorimpl) != 0 && (child$ui_release instanceof PointerInputModifierNode) && ((PointerInputModifierNode) child$ui_release).sharePointerInputWithSiblings()) {
                    return true;
                }
            }
        }
