...
public abstract class Ripple implements Indication {
    ...
    public abstract RippleIndicationInstance mo888rememberUpdatedRippleInstance942rkJo(InteractionSource interactionSource, boolean z, float f, State<Color> state, State<RippleAlpha> state2, Composer composer, int i);
    ...
    public final IndicationInstance rememberUpdatedInstance(InteractionSource interactionSource, Composer composer, int i) {
        ...
        if (this.color.getValue().m2566unboximpl() != Color.Companion.m2592getUnspecified0d7_KjU()) {
            mo890defaultColorWaAFU9c = this.color.getValue().m2566unboximpl();
        } else {
            mo890defaultColorWaAFU9c = rippleTheme.mo890defaultColorWaAFU9c(composer, 0);
        }
        composer.endReplaceableGroup();
        RippleIndicationInstance mo888rememberUpdatedRippleInstance942rkJo = mo888rememberUpdatedRippleInstance942rkJo(interactionSource, this.bounded, this.radius, SnapshotStateKt.rememberUpdatedState(Color.m2546boximpl(mo890defaultColorWaAFU9c), composer, 0), SnapshotStateKt.rememberUpdatedState(rippleTheme.rippleAlpha(composer, 0), composer, 0), composer, (i & 14) | ((i << 12) & 458752));
        EffectsKt.LaunchedEffect(mo888rememberUpdatedRippleInstance942rkJo, interactionSource, new Ripple$rememberUpdatedInstance$1(interactionSource, mo888rememberUpdatedRippleInstance942rkJo, null), composer, ((i << 3) & androidx.appcompat.R.styleable.AppCompatTheme_toolbarNavigationButtonStyle) | 520);
        if (ComposerKt.isTraceInProgress()) {
            ComposerKt.traceEventEnd();
        }
        composer.endReplaceableGroup();
        return mo888rememberUpdatedRippleInstance942rkJo;
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (obj instanceof Ripple) {
            ...
            return this.bounded == ripple.bounded && Dp.m5055equalsimpl0(this.radius, ripple.radius) && Intrinsics.areEqual(this.color, ripple.color);
        }
        return false;
    }

    public int hashCode() {
        return (((Boolean.hashCode(this.bounded) * 31) + Dp.m5056hashCodeimpl(this.radius)) * 31) + this.color.hashCode();
    }
}
