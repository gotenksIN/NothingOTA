...
public final class AndroidRippleIndicationInstance extends RippleIndicationInstance implements RememberObserver {
    ...
    private AndroidRippleIndicationInstance(boolean z, float f, State<Color> state, State<RippleAlpha> state2, RippleContainer rippleContainer) {
        ...
        this.rippleSize = Size.Companion.m2396getZeroNHjbRc();
        this.rippleRadius = -1;
    ...
    public void drawIndication(ContentDrawScope contentDrawScope) {
        ...
        this.rippleSize = contentDrawScope.mo3096getSizeNHjbRc();
        if (Float.isNaN(this.radius)) {
            i = MathKt.roundToInt(RippleAnimationKt.m892getRippleEndRadiuscSwnlzA(contentDrawScope, this.bounded, contentDrawScope.mo3096getSizeNHjbRc()));
        } else {
            i = contentDrawScope.mo296roundToPx0680j_4(this.radius);
        }
        this.rippleRadius = i;
        long m2566unboximpl = this.color.getValue().m2566unboximpl();
        float pressedAlpha = this.rippleAlpha.getValue().getPressedAlpha();
        contentDrawScope.drawContent();
        ContentDrawScope contentDrawScope2 = contentDrawScope;
        m897drawStateLayerH2RKhps(contentDrawScope2, this.radius, m2566unboximpl);
        Canvas canvas = contentDrawScope2.getDrawContext().getCanvas();
        getInvalidateTick();
        RippleHostView rippleHostView = getRippleHostView();
        if (rippleHostView != null) {
            rippleHostView.m896updateRipplePropertiesbiQXAtU(contentDrawScope.mo3096getSizeNHjbRc(), this.rippleRadius, m2566unboximpl, pressedAlpha);
    ...
    public void addRipple(PressInteraction.Press interaction, CoroutineScope scope) {
        ...
        rippleHostView.m895addRippleKOepWvA(interaction, this.bounded, this.rippleSize, this.rippleRadius, this.color.getValue().m2566unboximpl(), this.rippleAlpha.getValue().getPressedAlpha(), this.onInvalidateRipple);
