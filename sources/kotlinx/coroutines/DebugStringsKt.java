...
public final class DebugStringsKt {
    ...
    public static final String toDebugString(Continuation<?> continuation) {
        String m5373constructorimpl;
        if (continuation instanceof DispatchedContinuation) {
            return continuation.toString();
        }
        try {
            Result.Companion companion = Result.Companion;
            m5373constructorimpl = Result.m5373constructorimpl(continuation + '@' + getHexAddress(continuation));
        } catch (Throwable th) {
        ...
        if (Result.m5376exceptionOrNullimpl(m5373constructorimpl) != null) {
            m5373constructorimpl = continuation.getClass().getName() + '@' + getHexAddress(continuation);
        }
